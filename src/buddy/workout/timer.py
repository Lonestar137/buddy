...
@click.option("-v", "--volume", type=float, default=0.1)
...
def timer(
    ...
    tones = [
        generate_tone(freq, duration, sample_rate, volume)
        for freq in melody_frequencies
    ]
    melody_length = len(melody_frequencies)

    print(
        f"Starting {round(runtime / 60, 2)}-minute melodic piano tone sequence..."
    )
    ...
    try:
        while elapsed < runtime:
            ...
            print(
                f"Playing note {note_index + 1}/{melody_length} (Freq: {current_freq:.2f} Hz) at {pretty_elapsed} minutes"
            )
            sd.play(tones[note_index], sample_rate)
            sd.wait()  # Wait for the tone to finish playing
            time.sleep(
                interval - duration
            )  # Adjust sleep to maintain 30s interval
            elapsed = time.time() - start_time
            note_index = (
                note_index + 1
            ) % melody_length  # Cycle through melody
